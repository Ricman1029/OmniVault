...
#| fig-cap: El vector gradiente es normal a la superficie de nivel $F(x,y,z) = x^2 + y^2 + z^2 = 4$

fig = go.Figure()

# Voy a graficar una esfera, esta conviene graficarla en coordenadas esfericas
# porque sino hay problemas con valores chicos en la raiz
...
# Curva C perteneciente a la superficie S
r = np.sqrt(3)                      # radio de la intersección de la esfera con x=1
theta = np.linspace(0, np.pi/2, 50)
y = r * np.cos(theta)
z = r * np.sin(theta)
x = np.full_like(y, 1)
...
# Recta derivada parcial en y
x0, y0 = 1, 1
z0 = np.sqrt(4 - x0**2 - y0**2)
df_dy = -y0/np.sqrt(4 - x0**2 - y0**2)
...
# Vector gradiente de la superficie
# Coordenadas del origen del vector
x = 1
y = 1
z = np.sqrt(2)
# Componentes del vector
u = (-x)/np.sqrt(4 - x**2 - y**2)
v = (-y)/np.sqrt(4 - x**2 - y**2)
w = -1
# Grafico el vector como un cono
...
- Calculemos ahora las segundas derivadas para poder armar el Hessiano:
  ...
  \end{aligned}
  $$
  Cuando analizamos la $\det{Hf}$ en el punto $(0,0)$ obtenemos $\det{Hf(0,0)} = -16 < 0$, y el teorema de la segunda derivada nos dice que el $f(0,0)=$ es un punto silla.
