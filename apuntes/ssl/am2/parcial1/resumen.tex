...
\vspace{0.2cm}
\fbox{\parbox{0.9\linewidth}{
\textbf{Taylor para varias variables}

Sea $ f:U \subset \mathbb{R}^{n} \to \mathbb{R}^{} $ con derivadas parciales continuas hasta de segundo orden al rededor del punto $ \vv{x_{0}}=(x_{0},\dots ,x_{n}) $. Entonces podemos escribir
\[
  f(\vv{x})=f(\vv{x_{0}}) + \sum_{i=1}^{n} (\vv{x}-\vv{x_{i}})\pdv[]{f}{x_{i}}\left(\vv{x_{0}}\right)+\frac{1}{2}\sum_{i,j=1}^{n} \left(\vv{x}-\vv{x_{i}}\right)\left(\vv{x}-\vv{x_{j}}\right)\frac{\partial^2f}{\partial x_{i}\partial x_{j}}\left(\vv{x_{0}}\right)+R_{2}(\Delta \vv{x},\vv{x_{0}})
\]
}}\vspace{0.2cm}
...
\vspace{0.2cm}
\fbox{\parbox{0.9\linewidth}{
\textbf{Taylor con matrices}

Sea $ f $ una funcion que va de $ \mathbb{R}^{2} \to \mathbb{R}^{} $, diferenciable en el punto $ (x_{0},y_{0}) $, podemos escribir el polinomio de Taylor de segundo orden como sigue
\[
f(x,y)=f(x_{0},y_{0}) + f_{x}(x_{0},y_{0})(\Delta x) + f_{y}(x_{0},y_{0})(\Delta y) + \begin{bmatrix}
	\Delta x & \Delta y
\end{bmatrix}\begin{bmatrix}
	f_{xx} & f_{xy} \\
	f_{yx} & f_{yy}
...
\vspace{0.2cm}
\fbox{\parbox{0.9\linewidth}{
\textbf{Teorema de la función implícita (una variable)}

Dada una función $ F $ definida en un disco $ D $ alrededor del punto $ (a,b) $, donde $ F(a,b)=0 $, $ F_{y}(a,b)\neq 0 $, y $ F_{x} $ y $ F_{y} $ son continuas en el disco, la ecuación $ F(x,y)=0 $ define a $ y $ como una función de $ x $ en el disco $ D $ con centro en $ (a,b) $, y la derivada de esta función está dada por la ecuación \ref{eq:derivacion-implicita} para cada $ x  \in D $.
}}\vspace{0.2cm}
...
\vspace{0.2cm}
\fbox{\parbox{0.9\linewidth}{
\textbf{Teorema de la función implícita (dos variables)}

Dada una función $ F $ definida en una bola alrededor del punto $ (a,b,c) $, donde $ F(a,b,c)=0 $, $ F_{z}(a,b,c)\neq 0 $, y $ F_{x} $, $ F_{y} $ y $ F_{z} $ son continuas en un disco $ D  \subseteq  \mathbb{R}^{2} $, la ecuación $ F(x,y,z)=0 $ define a $ z $ como una función de $ (x,y) $ en el disco $ D $, y las derivadas parciales de esta función están dadas por las ecuaciones \ref{eq:derivacion-implicita-2} para cada $ (x,y) \in D $.
}}\vspace{0.2cm}
