...
// ===========================
const char *ssid = "avc648554";
const char *password = "22580660";
...
void tomar_y_enviar_foto() {
  ...
  // Si estoy conectado le aviso al servidor que voy a mandar info
  if (WiFi.status() == WL_CONNECTED) {
    HTTPClient http;
    http.begin(servidor);
    http.addHeader("Content-Type", "application/octet-stream");

    // Mando la foto tomada
    int httpResponseCode = http.POST(fb->buf, fb->len);
    Serial.printf("Código HTTP: %d\n", httpResponseCode);
    http.end();
  } else {
    Serial.println("Error de conexión Wi-Fi");  // No estoy conectado al Wi-Fi
  }
